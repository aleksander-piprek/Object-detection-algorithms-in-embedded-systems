...
int imageInference(const std::string& inputPath) 
{
    ...
    // // Use new ONNX Runtime API for input/output names
...
{
    // Init ONNX & Session (same as before)
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "yolov5");
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    ...
    cuda_options.device_id = 0;  // GPU ID 0
    ...
    while (cap.read(frame)) {
        ...
        // Preprocess frame to CHW format
        std::vector<float> input_tensor_values(INPUT_WIDTH * INPUT_HEIGHT * 3);
        int idx = 0;
        for (int c = 0; c < 3; ++c) {
            for (int y = 0; y < INPUT_HEIGHT; ++y) {
        ...
        for (size_t i = 0; i < num_detections; ++i) {
            ...
            if (max_score * obj_conf < CONFIDENCE_THRESHOLD) continue;
            ...
            cv::putText(frame, std::to_string(class_id), cv::Point(left, top - 5),
